...
export const useRemoteShapeProps = () => {
    ...
    const setSides = useShapeStore(state => state.setSides);
    ...
    const onSuccess = (data: ShapeProperties) => {
        ...
        setData(data)
        setIsSuccess(true);
        trigger === 0 ? toast.success("connection established") : toast.success("box successfully updated!")
    }
    const onError = (error: PostgrestError) => {
        setIsSuccess(false);
        toast.error(`something went wrong, analog state is used, new values may not be saved \n additional : ${error.message}`, {duration: 5000})
